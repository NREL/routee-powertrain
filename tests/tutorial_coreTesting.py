...
def remove_nan(df_passes, nan_threshold=1000000):
    ...
    val = (
        nan_list > nan_threshold
    ).values  # make a list of nan val greater than the threshold, i.e. inadmissible features --> 1M
...
xgb_model = Model(
    estimator=XGBoost(feature_pack, predict_type=option), description=scenario_name
)
...
rf_model = Model(
    estimator=RandomForest(feature_pack, predict_type=option), description=scenario_name
)
# training model with data (also does validation with test data and calculates error metrics)
        ...
        for veh in veh_list:
            ...
            for trip in trip_list:
                ...
                df_trip = df_trip.sort_values(
                    by=["time_local_start"]
                )  # properly sorting the veh data by start time
...
plt.plot(links_df["distance"], xgboost_output * 100 / links_df.miles, label="xgb")

plt.plot(links_df["distance"], rf_output * 100 / links_df.miles, label="RF")
plt.plot(links_df["distance"], links_df["gge"] * 100 / links_df.miles, label="FastSIM")
...
plot_prediction(
    links_df,
    ["xgb", "rf", "eb"],
    [xgboost_output, rf_output, baseline_output],
    plot_features,
)
...
links_df["miles"] = [0.1] * len(links_df)
links_df["distance"] = links_df["miles"].cumsum()
...
features = (Feature("gpsspeed", units="mph"), Feature("grade", units="ratio"))
