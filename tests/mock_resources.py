...
def mock_route() -> pd.DataFrame:
    route_path = test_dir() / Path("routee-powertrain-test-data") / Path(
        "trip_11394_1_metropia_austin_v2.csv")
...
def mock_ev_data():
    path = test_dir() / Path("routee-powertrain-test-data") / Path(
        "test_data_2016_Nissan_Leaf_30_kWh.csv")
    return pd.read_csv(path)


def mock_ice_data():
    path = test_dir() / Path("routee-powertrain-test-data") / Path(
        "test_data_2016_TOYOTA_Corolla_4cyl_2WD.csv")
    return pd.read_csv(path)
