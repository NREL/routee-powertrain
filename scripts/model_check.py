...
eb_model_list = [x for x in os.listdir(PATH_to_models) if x.endswith('ExplicitBin.json')]
rf_model_list = [x for x in os.listdir(PATH_to_models) if x.endswith('RandomForest.json')]
...
for basename_i in root_list:
    ...
    eb_model_error_dwrpe.append(eb_model.metadata['errors']['distance_weighted_relative_percent_difference'])
    ...
    rf_model_error_dwrpe.append(rf_model.metadata['errors']['distance_weighted_relative_percent_difference'])
